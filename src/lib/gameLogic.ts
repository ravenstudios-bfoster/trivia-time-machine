...
export const calculatePoints = (question: Question, timeRemaining: number, usedHint: boolean): number => {
...
export const checkAnswer = (question: Question, selectedOptionId?: string, writtenAnswer?: string): boolean => {
...
export const processAnswer = (question: Question, selectedOptionId?: string, writtenAnswer?: string, timeRemaining: number = 0, usedHint: boolean = false): PlayerAnswer => {
...
export const createTimer = (duration: number, onTick: (timeRemaining: number) => void, onComplete: () => void) => {
...
export const getLevelDescription = (level: Level): string => {
  switch (level) {
    ...
    case 2:
      return "Official Fan - Seen the movie countless times, can quote dialog, AND watched some of the DVD's Special Features";
    case 3:
      return "THE BIGGEST FAN - Still watch the movie on regular basis, have watched every second of behind the scenes footage from DVD or any source you can find, watched TV specials, Rode the ride, Played the video game, etc - Yep YOU'RE A SUPER NERD!";
    default:
      return "";
  }
};
